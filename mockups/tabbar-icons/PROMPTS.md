...
Ten directions for the bottom bar (Home, Block, the centre plus, Friends,
...
**The centre button is green, not gold** (Melvin, same day: "we actually
dont want the plus to be gold ... like the green from the buttons in the
onboarding"): `OnboardingGreen.fill` #5AB950 with its darker edge
`OnboardingGreen.shade` #418E3A and a white glyph, exactly the onboarding
buttons. No gold anywhere in the bar.
...
> picture-book style. Palette: cream #FEF9F2, sky blue #3F80AD, deep teal
> #3E8175, brown ink #413024, and one green for the main action: #5AB950
> with a darker edge of #418E3A and a white glyph on it. Type is rounded,
> like SF Pro Rounded. No gold or yellow in the bar.
>
> The bar has five tabs, left to right: Home; Block (Otto holds your
> distracting apps until you meditate); a larger round centre button that
> starts a meditation, which is the one green (#5AB950) element; Friends;
> Profile. Each side tab has an icon and a label: Home, Block, Friends,
> Profile. The centre button has no label.
>
...
1. **The bar is the meadow.** The bar itself is a strip of grass and
   ...
   grass: a cottage (Home), a wooden gate (Block), a round green hill-shaped
   mound with a white plus as the centre button, two tree stumps side by
   side (Friends), a signpost with a name board (Profile). The selected
   object stands in a soft beam of sunlight.
2. **Otto is every icon.** Tiny Otto figures in simple poses: Otto sitting
   ...
   meditating silhouette in white inside the green centre circle, two Ottos
   shoulder to shoulder (Friends), Otto wearing a name badge (Profile).
   Selected is full colour; unselected is a soft single-colour silhouette.
   Think Finch, whose world is its pet.
3. **Chunky and lifted, Duolingo style.** Bold, rounded, filled icons with a
   thick darker lip underneath so they look pressable. The centre button is
   exactly the onboarding button: #5AB950 with a #418E3A lip and a white
   plus. The selected tab sits in a rounded square with a 2pt outline in its
   own colour.
4. **Liquid Glass.** iOS 26 style: the bar is a floating translucent glass
   ...
   button is its own separate green glass orb, like a dewdrop, floating
   beside the capsule with a white plus.
5. **Ensō brush.** Japanese sumi-e ink brush marks on cream, each icon one or
   ...
   (Block), a brushed ensō filled in green with a white plus (centre), two
   stacked stones in a cairn (Friends), a single leaf (Profile). The
   selected icon fills with a soft watercolour wash.
6. **Sky phases.** The icons are the sky at different times: sun over two
   hills (Home), a crescent moon, rest and holding (Block), a white rising
   sun on the green centre disc, two stars joined by a faint line like a
   tiny constellation (Friends), one bright star (Profile). Unselected are
   pale outlines; the selected one glows softly.
7. **Garden that grows.** A plant at different stages: a sprout in a small
   clay pot (Home), a little fence around a flower (Block), a white
   seedling on the green centre button, two flowers leaning together
   (Friends), a young tree (Profile).
   Unselected icons are closed buds; the selected one blooms into full
   colour. Think Forest, where focus grows a tree.
8. **Game tiles that lift.** Like Clash Royale's bottom bar: each tab is a
   ...
   with an open paw (Block), a glowing green gem with a white plus
   (centre), two banners (Friends), a round crest with a leaf (Profile).
9. **Sticker book.** Every icon is a die-cut sticker with a thick white
   ...
   raised paw, a green lotus with a white plus, two smiling faces, Otto's
   head. The selected sticker straightens, grows slightly and gains a colour
   background; the others stay tilted and slightly faded.
10. **Soft clay 3D.** Chunky, rounded, matte clay-like 3D objects with soft
    lighting, like hand-made toys: a little house, a hand held up, a green
    clay button with a white plus, two pebbles with faces, a small sloth
    head. Selected objects are full colour; unselected are pale, same-colour
    clay.
...
assets at 1x, 2x and 3x. Changing the plus from gold to green also changes
the colour grammar recorded in CLAUDE.md ("the only gold object on the
bar"); record it there when it ships.
